...
extension Parsing {
    ...
    /// let choice = try await Effect.perform(Parsing.Backtrack(
    ...
    ///     typealias Handled = Parsing.Backtrack<MyInput, MyOutput>
    ///
    ///     func handle(
    ///         _ effect: Handled,
    ///         continuation: consuming Effect.Continuation.One<MyOutput, Parsing.Error>
    ///     ) async {
    ...
    ///             }
    ///         }
    ///         await continuation.resume(throwing: Parsing.Error.noAlternativeMatched)
    ///     }
    /// }
    /// ```
    ///
    ...
    public struct Backtrack<Input: Parsing.Input, Output: Sendable, E: Swift.Error & Sendable>: Effect.`Protocol` {
